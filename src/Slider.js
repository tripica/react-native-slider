...
type Props = {
  ...
  graduationLabelStyle: StyleProp<TextStyle>,
  graduationLabelContainerStyle: any,
...
class Slider extends React.Component<Props> {
  ...
  getInitialState(props) {
    const { graduation, maximumValue, minimumValue } = props;

    const numberOfGraduations = graduation
      ? (maximumValue - minimumValue) / graduation + 1
      : 0;
  ...
  shouldComponentUpdate = function (nextProps, nextState) {
    ...
    return (
      ...
      !styleEqual(
        this.props.graduationLabelContainerStyle,
        nextProps.graduationLabelContainerStyle
      ) ||
      !styleEqual(
        this.props.graduationLabelStyle,
        nextProps.graduationLabelStyle
      )
    );
  };
  ...
  render() {
    ...
    const currentValueBubbleOverflow =
      currentValueBubbleSize.width / 2 - thumbSize.width / 2;
    const outputRangeMin = -currentValueBubbleOverflow;
    const outputRangeMax =
      containerSize.width -
      (currentValueBubbleSize.width - currentValueBubbleOverflow);

    const currentValueBubbleLeft =
      !!this.props.currentValueBubble &&
      value.interpolate({
        inputRange: [minimumValue, maximumValue],
    ...
    var numberOfGraduations = graduation
      ? (maximumValue - minimumValue) / graduation + 1
      : 0;

    // ES6 Array.prototype.keys is broken in Android as 08/2017
    let graduationArray = [];
    Array(numberOfGraduations)
      .fill(0)
      .forEach((value, index) => graduationArray.push(index));
        ...
        {graduationArray.map((i) =>
          !ignoredGraduations || !ignoredGraduations.includes(i + 1) ? (
            <View
              key={i}
              style={{
                top: containerSize.height / 2 + trackSize.height / 2,
                position: "absolute",
              }}
            >
              <View
              ...
              <Animated.View
                ...
                style={[
                  mainStyles.graduationLabel,
                  graduationLabelContainerStyle,
                  {
                    width: this.state.legendWidth[i],
                    left:
                      this._getGraduationOffset(i) -
                      this.state.legendWidth[i] / 2,
                  },
                ]}
              >
            ...
            showBarAtIgnoredGraduation && (
              ...
              >
                <View
                  style={{
                    backgroundColor: maximumTrackTintColor,
                    marginTop: -(trackSize.height + GRADUATION_HEIGHT - 2) / 2,
                    width: 2,
                    height: GRADUATION_HEIGHT - 2,
                    left: this._getGraduationOffset(i),
                    ...valueVisibleStyle,
                  }}
                />
              </View>
        ...
        {!!this.props.currentValueBubble && !!this.props.graduationLabel && (
          <Animated.View
            ...
            style={[
              ...
              mainStyles.currentValueBubbleContainer,
              currentValueBubbleContainerStyle,
              { left: currentValueBubbleLeft, ...valueVisibleStyle },
            ]}
          >
            <Text
              style={[
                mainStyles.currentValueBubble,
                currentValueBubbleTextStyle,
              ]}
            >
              {this.props.graduationLabel(this.props.value)}
            </Text>
          </Animated.View>
        )}
      </View>
  ...
  _getGraduationOffset = (index: number) => {
    ...
    var { containerSize, thumbSize } = this.state;

    var graduationOffset = thumbSize.width / 2;

    graduationOffset +=
      ((minimumValue + graduation * index) *
    ...
    const graduationStyleObject =
      graduationStyle && StyleSheet.flatten(graduationStyle);
    graduationOffset -=
      graduationStyleObject && graduationStyleObject.width
        ? graduationStyleObject.width / 2
        : GRADUATION_WIDTH / 2;
  ...
  _handleMeasure = (name: string, x: Object) => {
    ...
    if (
      ...
      this._trackSize &&
      this._thumbSize &&
      ((this.props.currentValueBubble && this._currentValueBubbleSize) ||
        !this.props.currentValueBubble)
    ) {
      this.setState({
  ...
  _renderGraduationLabel = (index: Number) => {
    if (this.props.graduationLabel) {
      return (
        <Text
          style={[{ textAlign: "center" }, this.props.graduationLabelStyle]}
        >
